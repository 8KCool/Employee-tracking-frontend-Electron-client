...
const startCapture = (e, employeeData, uploadToS3) => {
    if (timerId == -1) {   // play capture
        ...
        getCurrentIdleState(employeeData, function (err, data) {
            if (err = "success") {
                startTime = new Date().getTime();
                currentState = data;
...
*/
let oldTime = 0;
const captureAuto = (employeeData, uploadToS3) => {
    for (var i = 0; i < currentState.LeftCaptureCnt; i++) {
        ...
        if (timerId != -1) {
            timeOutArray.push(setTimeout(() => {
                ...
                if (timerId != -1) {
                    captureScreen(employeeData.id, uploadToS3);
                }
            }, timeOut * 1000));
            oldTime = timeOut;
        }
